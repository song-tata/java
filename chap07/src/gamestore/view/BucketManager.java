...
public class BucketManager implements  Manager{
	...
	public void remove() {
		...
		id = in.next();
		for(Game g : GameManager.getWishList()) {
			if(g.getProductID().equals(id))
			GameManager.getWishList().remove(g);
			break;
			}
	...
	public void showbasket() {
		...
		for(Game g : GameManager.getWishList()) {
			System.out.printf("%s10 | %s | %s | %d 개 | %d\n ",g.getProductName(),g.getProductID(),g.getProductGenre(),g.getcount,g.price);
			gameKindsCount++;
			
		}
		System.out.println(gameKindsCount+" 종류의 게임이 담겨있습니다.");
		System.out.println("상품의 총 갯수 : "+gameKindsCount);
		 gameKindsCount = 0;
